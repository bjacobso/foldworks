...
export const view = defineView<Model, Message, { isDark: boolean }>((model, { isDark }, h) => {
  ...
  return h.div([h.Class("code-demo"), h.DataAttribute("code-editor-demo", "true")], [
    h.div([h.Class("code-demo__intro")], [
      ...
      h.p([], ["A native Foldkit editor with highlighting, search, suggestions, and live JSON feedback. Inspect the editor state below, or load a larger document to explore visible-line rendering."]),
    ]),
    h.div([h.Class("code-demo__toolbar")], [
      Select.control({ ariaLabel: "Example language", value: model.editor.document.languageId,
        options: [{ value: "json", label: "JSON configuration" }, { value: "typescript", label: "TypeScript" }, { value: "text", label: "Plain text" }],
    ...
    h.p([], ["Highlighting uses a small lexer, suggestions use document words, and JSON validation supplies squiggles. Multi-cursor editing, semantic language services, and full international text layout remain future work."]),
